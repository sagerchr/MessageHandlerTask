...
osThreadId MessageHandlerTaskHandle;
/* USER CODE END PV */
...
void MessageHandlerTask(void const * argument);
/* USER CODE BEGIN PFP */
...
printf("TEST");
  ...
  /* USER CODE BEGIN RTOS_THREADS */
	...
	osThreadDef(MessageHandlerTask, MessageHandlerTask, osPriorityNormal , 0, 2048);
	MessageHandlerTaskHandle = osThreadCreate(osThread(MessageHandlerTask), NULL);

	osThreadDef(dspTask, dspTask,  osPriorityHigh , 0, 1024);
	dspTaskHandle = osThreadCreate(osThread(dspTask), NULL);

	osThreadDef(lwIPTask, lwIPTask, osPriorityNormal, 0,2048);
